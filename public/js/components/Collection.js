import { MissingCard } from "./Card.js";
...
<div v-if="collectionStats">
	...
	<select v-model="selectedSetCode">
		<option v-for="set in collectionStats" :value="set.name">{{ set.fullName }}</option>
	</select>
	<div class="set-stats">
		<div v-if="selectedSet">
			<table>
				...
				<tr v-for="r in ['common', 'uncommon', 'rare', 'mythic']" v-if="selectedSet[r + 'Count'] && selectedSet['total'][r + 'Count'] > 0">
					<td>{{r}}</td>
			...
			<h3>
				...
				<select v-model="missingCardsRarity">
					<option value="common">Commons</option><option value="uncommon">Uncommons</option><option value="rare">Rares</option><option value="mythic">Mythics</option>
				</select>
				<input type="checkbox" id="show-non-booster" v-model="showNonBooster"><label for="show-non-booster">Show non-booster cards</label>
			</h3>
			<div class="card-container">
				<missing-card v-for="(card, index) in selectedSet.cards.filter(c => c.rarity == missingCardsRarity && (showNonBooster || c.in_booster) && c.count < 4)" :key="card.uniqueID" :card="card" :language="$root.language"></missing-card>
			</div>
		</div>
	</div>
</div>
<div v-else>
	Collection statistics not available.
</div>`,
	props: {
