...
describe("Grid Draft", function () {
	for (const regularBoosters of [false, true]) {
		describe(`Regular Boosters: ${regularBoosters}`, function () {
			...
			const startDraft = (
				...
				boosterCount: number = 18,
				twoPicksPerGrid: boolean = false
			) => {
			...
			describe("2 Players, 2 picks per grid", function () {
				startDraft(regularBoosters, 12, true);
				endDraft();
			});
