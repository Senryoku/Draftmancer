...
describe("Collection Restriction", function() {
	...
	it(`Submit random collections.`, function(done) {
		...
		for(let cid of MTGAIDs) {
			for (let c = 0; c < clients.length; ++c) {
				...
				if(cardCount > 0)
					collections[c][cid] = cardCount;
			}
		}

		let sendCollections = 0;
		for (let c = 0; c < clients.length; ++c) {
			clients[c].emit("setCollection", collections[c], () => { ++sendCollections; if(sendCollections === clients.length) done(); });
		}
	});
	...
	it("Select ZNR as a known MTGA set.", function(done) {
		ownerIdx = clients.findIndex(c => c.query.userID == Sessions[sessionID].owner);

		clients[(ownerIdx + 1) % clients.length].once("setRestriction", () => { done(); });
		clients[ownerIdx].emit("setRestriction", ["znr"]);
	});
