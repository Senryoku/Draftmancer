...
export default {
  ...
  data: () => {
    return {
      ...
      //
      selectedCube: Constant.CubeLists.length > 0 ? Constant.CubeLists[0] :
                                                    null,
  ...
  methods: {
    initializeSocket: function() {
      ...
      this.socket.on("chatMessage", message => {
        ...
        bubble.timeoutHandler =
            window.setTimeout(() => (bubble.style.opacity = 0), 5000);
      });

      this.socket.on(
          "publicSessions", sessions => { this.publicSessions = sessions; });
        ...
        if (this.winstonDraftState || this.gridDraftState ||
          ...
          if (this.userID === this.sessionOwner) {
            Swal.fire({
                  ...
                  text:
                      `Wait for ${data.disconnectedUserNames.join(", ")} to come back or...`,
          ...
          if (this.userID === this.sessionOwner) {
            Swal.fire({
                ...
                .then(result => {
                  if (result.value)
                    this.socket.emit("replaceDisconnectedPlayers");
                });
          } else {
            Swal.fire({
              position: "center",
      ...
      this.socket.on("updateUser", data => {
        ...
        if (!user) {
          if (data.userID === this.sessionOwner &&
              data.updatedProperties.userName)
      ...
      this.socket.on("isPublic", data => { this.isPublic = data; });
      ...
      this.socket.on("bots", data => { this.bots = parseInt(data); });
      ...
      this.socket.on("setPickTimer", timer => { this.maxTimer = timer; });
      ...
      this.socket.on("readyCheck", () => {
        ...
        if (this.enableNotifications) {
          new Notification("Are you ready?", {
            body:
                `${this.userByID[this.sessionOwner].userName} has initiated a ready check`,
          });
        }

        const ownerUsername = this.sessionOwner in this.userByID ?
            this.userByID[this.sessionOwner].userName :
            this.sessionOwnerUsername ? this.sessionOwnerUsername :
                                        "Session owner";
      ...
      this.socket.on("setReady", (userID, readyState) => {
        ...
        if (userID in this.userByID)
          this.userByID[userID].readyState = readyState;
      ...
      this.socket.on("winstonDraftRandomCard", cid => {
        ...
        const cardView =
            new ComponentClass({parent: this, propsData: {card: c}});
        cardView.$mount();
        Swal.fire({
          position: "center",
          title:
              `You drew ${Cards[cid].printed_name[this.language]} from the card pool!`,
          html: cardView.$el,
          customClass: SwalCustomClasses,
          showConfirmButton: true,
        });
      ...
      this.socket.on("rejoinWinstonDraft", data => {
        ...
        if (this.userID === data.state.currentPlayer)
          this.draftingState = DraftState.WinstonPicking;
        else
          this.draftingState = DraftState.WinstonWaiting;
      ...
      this.socket.on("startGridDraft", state => {
        setCookie("userID", this.userID);
        this.drafting = true;
        this.draftingState = this.userID === state.currentPlayer ?
            DraftState.GridPicking :
            DraftState.GridWaiting;
      ...
      this.socket.on("startRochesterDraft", state => {
        ...
        this.drafting = true;
        this.draftingState = this.userID === state.currentPlayer ?
            DraftState.RochesterPicking :
            DraftState.RochesterWaiting;
      ...
      this.socket.on("rejoinDraft", data => {
        ...
        this.$nextTick(() => {
          if (typeof this.$refs.deckDisplay !== "undefined")
            this.$refs.deckDisplay.sync();
          if (typeof this.$refs.sideboardDisplay !== "undefined")
            this.$refs.sideboardDisplay.sync();
        });
      ...
      this.socket.on("pickAlert", data => {
        fireToast(
            "info",
            `${data.userName} picked ${Cards[data.cardID].printed_name[this.language]}!`);
      });
      ...
      this.socket.on("disableTimer", () => { this.pickTimer = -1; });
    },
    ...
    burnCard: function(e, c) {
      ...
      if (this.burningCards.length > this.burnedCardsPerRound)
        this.burningCards.shift();
      if (e) e.stopPropagation();
    },
    restoreCard: function(e, c) {
      if (!this.burningCards.includes(c)) return;
      this.burningCards.splice(this.burningCards.findIndex(o => o === c), 1);
      if (e) e.stopPropagation();
    },
    ...
    dragBoosterCard: function(e, card) {
      e.dataTransfer.setData(
          "isboostercard", "true");  // Workaround: See allowBoosterCardDrop
    ...
    dropBoosterCard: function(e, options) {
      ...
      // buuuuut...)
      if (e.dataTransfer.getData("isboostercard") !== "true" ||
          (this.draftingState != DraftState.Picking &&
           this.draftingState != DraftState.RochesterPicking))
        return;
      e.preventDefault();
      ...
      if (!this.selectedCard) {
        console.error(
            `dropBoosterCard error: this.selectedCard === ${this.selectedCard}`);
        return;
      }
    ...
    pickCard: function(options) {
      if (this.pickInFlight ||  // We already send a pick request and are
      ...
      this.$nextTick(() => {
        if (this.rochesterDraftState) {
          this.socket.emit(
              "rochesterDraftPick", this.rochesterDraftState.booster.findIndex(
                                        c => c === this.selectedCard),
              answer => {
                this.pickInFlight = false;
                if (answer.code !== 0)
                  alert(`pickCard: Unexpected answer: ${answer.error}`);
              });
          this.draftingState = DraftState.RochesterWaiting;
        } else {
          this.socket.emit(
        ...
        if (options && options.toSideboard)
          this.addToSideboard(this.selectedCard);
        else
    ...
    forcePick: function() {
      ...
      while (1 + this.burningCards.length < this.booster.length &&
             this.burningCards.length < this.burnedCardsPerRound) {
        let randomIdx;
        do
          randomIdx = Math.floor(Math.random() * this.booster.length);
        while (this.booster[randomIdx] === this.selectedCard ||
    ...
    startWinstonDraft: async function() {
      ...
      const {value: boosterCount} = await Swal.fire({
        title: "Winston Draft",
        html:
            `<p>Winston Draft is a draft variant for two players. <a href="https://mtg.gamepedia.com/Winston_Draft" target="_blank">More information here</a>.</p>How many boosters for the main stack (default is 6)?`,
    ...
    winstonDraftTakePile: function() {
      const cards =
          this.winstonDraftState.piles[this.winstonDraftState.currentPile];
    ...
    setGridDraftState: function(state) {
      const prevBooster =
          this.gridDraftState ? this.gridDraftState.booster : null;
      this.gridDraftState = state;
      const booster = [];
      let idx = 0;
      for (let cid of this.gridDraftState.booster) {
        if (cid in Cards) {
          if (prevBooster && prevBooster[idx] && prevBooster[idx].id === cid)
            booster.push(prevBooster[idx]);
          else
            booster.push(genCard(cid));
        } else
          booster.push(null);
        ++idx;
      }
    ...
    setRochesterDraftState: function(state) {
      let booster = [];
      if (this.rochesterDraftState && this.rochesterDraftState.booster &&
          this.rochesterDraftState.booster.length - 1 ===
              state.booster.length) {
        booster = this.rochesterDraftState.booster.filter(
            c => state.booster.includes(c.id));
      } else
    ...
    parseMTGALog: function(e) {
      ...
      reader.onload = async e => {
          ...
          if (m) {
            ...
            if (name != this.userName) {
              const swalResult = await Swal.fire({
                ...
                title: "User Name",
                text:
                    `Found display name '${name}', do you want to use it as your User Name?`,
                customClass: SwalCustomClasses,
                showCancelButton: true,
                showConfirmButton: true,
                confirmButtonColor: "#3085d6",
        ...
        if (contents.indexOf("DETAILED LOGS: DISABLED") !== -1 &&
        ...
        let playerIds = new Set(
            Array.from(contents.matchAll(/"playerId":"([^"]+)"/g))
                .map(e => e[1]));
        ...
        if (playerIds.size > 1) {
          const swalResult = await Swal.fire({
            ...
            text:
                `Looks like there are collections from multiple accounts (${playerIds.size}) in these logs, do you want to intersect them all, or just import the latest?`,
            customClass: SwalCustomClasses,
            showCancelButton: true,
            showConfirmButton: true,
            confirmButtonColor: "#3085d6",
          ...
          if (swalResult.value) {
            ...
            for (let pid of playerIds) {
              const startIdx =
                  contents.lastIndexOf(`"payload":{"playerId":"${pid}"`);
            ...
            for (let i = 1; i < collections.length; ++i)
              cardids = Object.keys(collections[i])
            ...
            for (let i = 1; i < collections.length; ++i)
              for (let id of cardids)
                collection[id] = Math.min(collection[id], collections[i][id]);
          } else
            collection = parseCollection(contents);
        } else
          collection = parseCollection(contents);
    ...
    parseCustomCardList: async function(file, options) {
      ...
      const parseLine = line => {
        ...
        let cardID = Object.keys(Cards).find(
            id => Cards[id].name == name && (!set || Cards[id].set === set) &&
        ...
        if (typeof cardID !== "undefined") {
          ...
          cardID = Object.keys(Cards).find(
              id => Cards[id].name.startsWith(name + " //") &&
        ...
        Swal.fire({
          ...
          text:
              `Could not find '${name}' in our database. (Note: this app only supports cards from MTG Arena.)`,
    ...
    exportDeck: function(full = true) {
      copyToClipboard(
          exportToMTGA(
              this.deck, this.sideboard, this.language, this.lands, full));
    ...
    toggleSetRestriction: function(code) {
      if (this.setRestriction.includes(code))
        this.setRestriction.splice(
            this.setRestriction.findIndex(c => c === code), 1);
      else
        this.setRestriction.push(code);
    },
    setSessionOwner: function(newOwnerID) {
      ...
      Swal.fire({
            title: "Are you sure?",
            text:
                `Do you want to surrender session ownership to ${user.userName}?`,
            icon: "warning",
            showCancelButton: true,
            customClass: SwalCustomClasses,
            confirmButtonColor: "#3085d6",
    ...
    removePlayer: function(userID) {
      ...
      Swal.fire({
            title: "Are you sure?",
            text:
                `Do you want to remove player '${user.userName}' from the session? They'll still be able to rejoin if they want.`,
            icon: "warning",
            showCancelButton: true,
            customClass: SwalCustomClasses,
            confirmButtonColor: "#d33",
    ...
    movePlayer: function(idx, dir) {
      ...
      [this.userOrder[idx], this.userOrder[other]] =
          [this.userOrder[other], this.userOrder[idx]];
      ...
      if (!storedDraftLog) {
        ...
        if (parsedLogs.sessionID !== this.sessionID) {
          Swal.fire({
            ...
            text:
                `Can't share logs: The session ID of your saved draft log ('${parsedLogs.sessionID}') doesn't match the id of yout current session ('${this.sessionID}').`,
            icon: "error",
            customClass: SwalCustomClasses,
          });
          return;
    ...
    generateBracket: function() {
      ...
      for (let i = 0; i < 8; ++i) {
        if (pairingOrder[i] < playerNames.length)
          players[i] = playerNames[pairingOrder[i]];
        else
          players[i] = "";
      }
    ...
    clearDeck() {
      this.deck = [];
      this.$nextTick(() => { this.$refs.deckDisplay.sync(); });
    },
    clearSideboard() {
      this.sideboard = [];
      this.$nextTick(() => { this.$refs.sideboardDisplay.sync(); });
    },
    updateAutoLands: function() {
      if (this.autoLand) {
        ...
        if (this.deck.length + addedLands > targetDeckSize) {
          let max = "W";
          for (let i = 0; i < this.deck.length + addedLands - targetDeckSize;
               ++i) {
            for (let c in this.lands)
          ...
          let min = "W";
          for (let i = 0; i < targetDeckSize - (this.deck.length + addedLands);
               ++i) {
            for (let c in this.lands)
              if (this.colorsInDeck[min] == 0 ||
    ...
    fetchTranslation: function(lang) {
      ...
      fetch(`data/MTGACards.${lang}.json`)
  ...
  computed: {
    ...
    winstonCanSkipPile: function() {
      ...
      return !(
          ...
          ((s.currentPile === 0 && !s.piles[1].length && !s.piles[2].length) ||
           (s.currentPile === 1 && !s.piles[2].length) || s.currentPile === 2));
    },
    virtualPlayers: function() {
      if (!this.drafting || !this.virtualPlayersData ||
    ...
    hasCollection: function() { return !isEmpty(this.collection);},

    colorsInDeck: function() { return this.colorsInCardPool(this.deck);},
    totalLands: function() {
      let addedLands = 0;
  ...
  mounted: async function() {
    ...
    try {
      ...
      const DefaultLoc =
          (await import("../public/data/MTGACards.en.json")).default;
      this.loadTranslation("en", DefaultLoc);
      if (!(this.language in this.loadedLanguages))
        this.fetchTranslation(this.language);
  ...
  watch: {
    sessionID: function() {
      ...
      history.replaceState(
          {sessionID: this.sessionID}, `MTGADraft Session ${this.sessionID}`,
    ...
    // Front-end options
    language: function() { setCookie("language", this.language);},
    ...
    boosterContent: {
      ...
      handler(val) {
        ...
        if (Object.values(val).reduce((acc, val) => acc + val) <= 0) {
          fireToast(
              "warning", "Your boosters should contain at least one card :)");
    ...
    customCardList: function() {
      if (this.userID != this.sessionOwner || !this.customCardList.length ||
          !this.socket)
        return;
      this.socket.emit("customCardList", this.customCardList, answer => {
        if (answer.code === 0) {
          fireToast(
              "success",
              `Card list uploaded (${this.customCardList.length} cards)`);
        } else {
          fireToast(
              "error", `Error while uploading card list: ${answer.error}`);
        }
      });
    },
