...
export class SpecialLandSlot extends BasicLandSlot {
    commonLandsIds;
    rate;
    landsToDistribute = new Map();
    constructor(set, commonLandsIds, rate, basicLandsIds) {
        super(set);
        this.commonLandsIds = commonLandsIds;
        this.rate = rate;
        if (basicLandsIds)
            this.basicLandsIds = basicLandsIds;
    }
    setup(commons) {
...
export const SpecialLandSlots = {
    ...
    ], 5 / 12, BasicLandIDs["khm"].filter((cid) => Cards[cid]["type"].startsWith("Basic Snow"))), // Always one snow-covered land, either basic or tapped. Shimmerdrift Vale (f09d98db-0176-41a7-b99b-ead29876cdab) appears in the common slot
};
export const BasicLandSlots = {};
for (let set in BasicLandIDs)
    BasicLandSlots[set] = new BasicLandSlot(set);
