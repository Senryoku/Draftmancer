...
export class Session {
    ...
    broadcastDisconnectedUsers() {
        ...
        this.forUsers(u => Connections[u]?.socket.emit("userDisconnected", {
            owner: this.owner,
    ...
    collection(inBoosterOnly = true) {
        ...
        if (!useCollection[0])
            if (inBoosterOnly)
                arrays.push(MTGACardIDs.filter(c => Cards[c].in_booster));
            else
    ...
    generateBoosters(boosterQuantity, options = {}) {
        ...
        if (this.useCustomCardList) {
            ...
            if (this.customCardList.customSheets) {
                ...
                const colorBalancedSlot = Object.keys(this.customCardList.cardsPerBooster).reduce((max, curr) => this.customCardList.cardsPerBooster[curr] > this.customCardList.cardsPerBooster[max] ? curr : max);
                // Do not color balance if we don't have at least a 5 cards slot
                const useColorBalance = this.colorBalance && this.customCardList.cardsPerBooster[colorBalancedSlot] >= 5;
                // Generate Boosters
                this.boosters = [];
                ...
                if (this.colorBalance) {
                    ...
                    for (let i = 0; i < boosterQuantity; ++i) {
                        ...
                        for (let j = 0; j < cardsPerBooster; ++j)
                            booster.push(pickCard(localCollection, booster));
                        this.boosters.push(booster);
                    }
                }
            }
            ...
            if (!options?.useCustomBoosters || customBoosters.some(v => v === "")) {
                ...
                if (acceptPaperBoosterFactories &&
                    ...
                    defaultFactory = getBoosterFactory(this.setRestriction.length === 1 ? this.setRestriction[0] : null, localCollection, defaultLandSlot, BoosterFactoryOptions);
            ...
            if (!boosterSpecificRules) {
                        ...
                        if (boosterSet === "") {
                            ...
                            if (!usedSets[boosterSet]) {
                                ...
                                if (acceptPaperBoosterFactories && isPaperBoosterFactoryAvailable(boosterSet)) {
                                    ...
                                    const landSlot = boosterSet in SpecialLandSlots
                                        ...
                                        : addLandSlot && !irregularSets.includes(boosterSet)
                                            ? BasicLandSlots[boosterSet]
                                                ...
                                                : defaultBasics
                                            : null;
                                    usedSets[boosterSet] = getBoosterFactory(boosterSet, this.setByRarity(boosterSet), landSlot, BoosterFactoryOptions);
                                    ...
                                    for (let slot of ["common", "uncommon", "rare"]) {
                                        if (countCards(usedSets[boosterSet].cardPool[slot]) <
                ...
                if (this.distributionMode === "shufflePlayerBoosters")
                    for (let rules of boosterFactories)
                        shuffleArray(rules);
                // Generate Boosters
                this.boosters = [];
                ...
                if (this.distributionMode === "shuffleBoosterPool")
                    shuffleArray(this.boosters);
    ...
    notifyUserChange() {
        ...
        this.forUsers(user => {
            if (Connections[user]) {
                Connections[user].socket.emit("sessionOwner", this.owner, this.owner in Connections ? Connections[this.owner].userName : null);
    ...
    winstonNextRound() {
        ...
        ++s.round;
        s.currentPile = 0;
        while (s.currentPile < 3 && !s.piles[s.currentPile].length)
            ++s.currentPile;
        if (s.currentPile >= 3) {
    ...
    winstonSkipPile() {
        ...
        if (s.currentPile === 2) {
            ...
            ++s.currentPile;
            if (s.piles[s.currentPile].length === 0)
                this.winstonSkipPile();
            else
                for (let user of this.users)
                    Connections[user].socket.emit("winstonDraftSync", s.syncData());
        }
    ...
    winstonTakePile() {
        ...
        Connections[s.currentPlayer()].pickedCards = Connections[s.currentPlayer()].pickedCards.concat(s.piles[s.currentPile]);
        if (s.cardPool.length > 0)
            s.piles[s.currentPile] = [s.cardPool.pop()];
        else
    ...
    gridDraftNextRound() {
        const s = this.draftState;
        if (!this.drafting || !s || !(s instanceof GridDraftState))
            return;
        ++s.round;
        if (s.round % 2 === 0) {
            ...
            for (let user of this.users)
                Connections[user].socket.emit("gridDraftNextRound", syncData);
            s.boosters.shift();
        ...
        for (let user of this.users)
            Connections[user].socket.emit("gridDraftNextRound", s.syncData());
    }
    ...
    rochesterDraftNextRound() {
        ...
        const syncData = s.syncData();
        for (let user of this.users)
            Connections[user].socket.emit("rochesterDraftNextRound", syncData);
    }
    ...
    startDraft() {
        ...
        for (let i = 0; i < this.bots; ++i)
            this.botsInstances.push(new Bot(`Bot #${i + 1}`, uuidv1()));
    ...
    pickCard(userID, pickedCards, burnedCards) {
        if (!this.drafting || this.draftState?.type !== "draft" || !this.users.has(userID))
            return;
        ...
        if (!pickedCards ||
            ...
            return reportError(1, `Session.pickCard: Invalid picked cards (pickedCards: ${pickedCards}, booster length: ${this.draftState.boosters[boosterIndex].length}).`);
        if (pickedCards.some(idx => idx >= this.draftState.boosters[boosterIndex].length))
            return reportError(3, `Session.pickCard: Invalid card index [${pickedCards.join(", ")}] for booster #${boosterIndex} (${this.draftState.boosters[boosterIndex].length}).`);
        ...
        if (burnedCards &&
            (burnedCards.length > this.burnedCardsPerRound ||
                burnedCards.length !==
                    Math.min(this.burnedCardsPerRound, this.draftState.boosters[boosterIndex].length - pickedCards.length) ||
                burnedCards.some(idx => idx >= this.draftState.boosters[boosterIndex].length)))
            return reportError(5, `Session.pickCard: Invalid burned cards (expected length: ${this.burnedCardsPerRound}, burnedCards: ${burnedCards.length}, booster: ${this.draftState.boosters[boosterIndex].length}).`);
        console.log(`Session ${this.id}: ${Connections[userID].userName} [${userID}] picked card '${pickedCards.map(idx => this.draftState.boosters[boosterIndex][idx].name)}' from booster #${boosterIndex}, burning ${burnedCards && burnedCards.length > 0 ? burnedCards.length : "nothing"}.`);
        for (let idx of pickedCards)
            Connections[userID].pickedCards.push(this.draftState.boosters[boosterIndex][idx]);
        ...
        for (let idx of cardsToRemove)
            this.draftState.boosters[boosterIndex].splice(idx, 1);
    ...
    nextBooster() {
        ...
        let index = 0;
        const boosterOffset = this.draftState.boosterNumber % 2 == 0 ? -this.draftState.pickNumber : this.draftState.pickNumber;
        let virtualPlayers = this.getSortedVirtualPlayers();
    ...
    resumeOnReconnection(msg) {
        ...
        this.forUsers(user => Connections[user]?.socket.emit("sessionOptions", {
            virtualPlayersData: this.getSortedVirtualPlayers(),
        }));
        if (!this.draftPaused && this.draftState instanceof DraftState)
            this.resumeCountdown();
        this.forUsers(u => Connections[u]?.socket.emit("resumeOnReconnection", {
            msg,
        }));
    }
    endDraft() {
    ...
    stopDraft() {
        if (!this.drafting || !this.draftState)
            return;
    ...
    pauseDraft() {
        if (!this.drafting || !this.countdownInterval)
            return;
    ...
    resumeDraft() {
        if (!this.drafting || !this.draftPaused)
            return;
        ...
        switch (this.draftLogRecipients) {
            ...
            case "delayed": {
                ...
                for (let u of this.users)
                    if (u !== this.owner)
                        Connections[u].socket.emit("draftLog", strippedLog);
                break;
            }
            case "everyone":
    ...
    replaceDisconnectedPlayers() {
        if (!this.drafting || !(this.draftState instanceof DraftState))
            return;
        ...
        for (let uid in this.disconnectedUsers) {
            ...
            if (!this.disconnectedUsers[uid].pickedThisRound) {
                const pickedCards = this.doBotPick(this.disconnectedUsers[uid].bot, this.disconnectedUsers[uid].boosterIndex);
    ...
    resumeCountdown() {
        ...
        if (this.maxTimer <= 0) {
            ...
            this.forUsers(u => Connections[u]?.socket.emit("timer", {
    ...
    getSortedVirtualPlayers() {
        if (this.botsInstances && this.botsInstances.length > 0) {
            ...
            return this.getSortedHumanPlayers();
    }
    emitMessage(title, text, showConfirmButton = true, timer = 1500) {
        this.forUsers(u => Connections[u]?.socket.emit("message", {
            title: title,
            text: text,
