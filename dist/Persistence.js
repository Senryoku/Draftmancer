...
async function tempDump(exitOnCompletion = false) {
    ...
    for (const sessionID in Sessions) {
        ...
        const PoDSession = {};
        for (let prop of Object.getOwnPropertyNames(s).filter(p => !["users", "countdownInterval", "botsInstances", "draftState"].includes(p))) {
            if (!(s[prop] instanceof Function))
                PoDSession[prop] = s[prop];
        }
        if (s.drafting) {
...
export function logSession(type, session) {
    ...
    if (session.customCardList && session.customCardList.name)
        mixdata.customCardListName = session.customCardList.name;
